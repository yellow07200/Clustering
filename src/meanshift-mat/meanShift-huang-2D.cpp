...
#define bandwidth 0.15 // 0.12-- marker
...
void Meanshift::eventsCallback_simple(const dvs_msgs::EventArray::ConstPtr& msg)
{
	...
	if (events_size>60)
	{
		...
		{
			ROS_INFO("beginEvent=%d, events_size=%d",beginEvent,events_size);
			std::cout<<"start.cluster_center.size()="<<cluster_center.size()<<std::endl;std::cout<<"aha2"<<std::endl;
			//points.clear();std::cout<<"aha3"<<std::endl;
			//temp_center.clear();
			centroid.clear();
			centroid_filter.clear();
			centroid_final.clear();
			//points_cluster.clear();std::cout<<"aha1"<<std::endl;
			points_vector.clear();
			points_ID0.clear(); // points in the first cluster;
			std::vector<std::vector<cv::Point> > clusters_test; //clusters_test.clear();
			int ClusterID=0;
			std::cout<<"aha"<<std::endl;
			//std::cout<<"loop in while"<<std::endl;
			counterIn = 0;
			counterOut = 0;
			std::vector<Point> points;std::cout<<"aha3"<<std::endl;
			cv::Mat data=cv::Mat(3, min(packet, events_size-beginEvent), CV_64F, cv::Scalar::all(0.));std::cout<<"aha6"<<std::endl;
			cv::Mat data_cluster=data;std::cout<<"aha5"<<std::endl;
			if(firstevent)
			{
			...
			//double usTime = 10.0;
			double ts;
			int temp=min(beginEvent+packet, events_size);std::cout<<"aha4"<<std::endl;
			...
			//std::cout<<"count_init="<<count_init<<std::endl;
			...
			if (centroid_filter.size()>0)
			{
				for(int i=0;i<centroid_filter.size();i++)
				{
					...
					for(int j=i+1;j<centroid_filter.size();j++)
					{
						...
						if (dis<(bandwidth/2)*(bandwidth/2)*240*180*1.2)// && x1!=0 && y1!=0)//800)// 400 last working //i!=j && 
						{
			...
			{ 
				...
				for (int tc=0; tc<cluster_center.size(); tc++)
				{
					...
					//int count_repeat=0;
					for (int tcf=tc+1; tcf<cluster_center.size(); tcf++)
					{
				...
				std::cout<<"cluster_center_mat.row="<<cluster_center_mat.rows<<",cluster_center_mat.cols="<<cluster_center_mat.cols<<std::endl; */
				
				// save in vector 
				std::cout<<"cluster_center.size()="<<cluster_center.size()<<std::endl;
				for (int tc=0; tc<cluster_center.size(); tc++)//cluster_center_mat.cols
				{
					int xxx=0; int yyy=0;
					//std::cout<<"tc="<<tc<<std::endl;
					int ccx=cluster_center[tc][0];//cluster_center_mat.at<double>(cv::Point(0, tc));//
					int ccy=cluster_center[tc][1];//cluster_center_mat.at<double>(cv::Point(1, tc));//
					...
					if (ccx!=0)
					{
						...
						cv_cluster_center_final.push_back(cv::Point(ccx,ccy));//(cv::Point(xxx,yyy));
				...
				std::cout<<"centroid_final.size="<<centroid_final.size()<<", cv_cluster_center_final.size="<<cluster_center.size()<<std::endl;
			...
			if (cv_cluster_center_final.size()>0)
			{
				...
				for (int w=0;w<cv_cluster_center_final_size;w++)//centroid_final //cluster_center
				{
					...
					for (int k=0;k<events_size;k++)
					{
						...
						// std::cout<<"disp1="<<distp<<std::endl;
						double distp=pow((x_mat-xw),2)+pow((y_mat-yw),2);
						//std::cout<<"disp="<<distp<<std::endl;
			...
			center_orien_pub.publish(cen_msg);ROS_INFO("angle3");
			//ROS_INFO("xxx1");
			//---------------Evaluation-----------------
			...
			cv::waitKey(1);*/
			ROS_INFO("aha");
			ROS_INFO("end-beginEvent=%d, events_size=%d",beginEvent,events_size);
		}//while packet
