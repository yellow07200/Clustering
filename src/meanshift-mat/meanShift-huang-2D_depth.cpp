...
#define bandwidth 0.5 // 0.12-- marker
...
void Meanshift::eventsCallback_simple(const dvs_msgs::EventArray::ConstPtr& msg)
{
	...
	if (events_size>60)
	{
		...
		double angle_ID0=0;
		double point_vector_depth_ave=0;
		...
		std::cout<<"cv_cluster_center_final.size()="<<cv_cluster_center_final.size()<<std::endl;
				...
				for (int k=0;k<events_size;k++)
				{
					...
					{	//points_vector.push_back(points[k]);
						...
						//if (ID==3) 
							...
							//std::cout<<"countt--1,"<<y_mat<<","<<x_mat<<std::endl;
							if (depth_sign==1)
							{
								int point_d=(int)depth_image.at<uchar>(y_mat, x_mat);
								double point_da= point_d/255*(1.2-0.3);
								points_vector_depth.push_back(point_da);
								point_vector_sum+=point_da;
								point_vector_ave=point_vector_sum/countt;
							}	
							//std::cout<<depth_sign<<std::endl;			
						...
						cv::circle(cv_image1.image, cv::Point(0,0), 0.2, color, 0.2, 1, 0);//cv::Scalar(ID_color,ID_color,ID_color)
						//std::cout<<"after-2"<<std::endl;   //x_mat,y_mat
						if (w==0)
						{
					...
					// if (w==0) 
					// {
					// 	point_vector_depth_ave=point_vector_ave;
					// 	std::cout<<"point_vector_depth_ave="<<point_vector_depth_ave<<std::endl;
					// }
