...
#define bandwidth 0.22//0.17//0.15
...
if (events_size>60)
{
	...
	while(beginEvent < events_size )
	{
        ...
        elapsedTime_one = std::chrono::duration_cast<std::chrono::duration<float>>(end - start).count();
	...
	ROS_INFO("cluster_points.size()=%d, centroid_final.size()=%d, clustCentX.size()=%d, centroid_prev.size()=%d", cluster_points.size(), centroid_final.size(), clustCentX.size(), centroid_prev.size());
	//std::cout<<"points_size="<<points.size()<<std::endl;
	//float Recall=TP/GT*100;
    ROS_INFO("elapsedTime=%f, total_events=%d, iter=%d", elapsedTime, total_events, iter);
    ROS_INFO("T/E=%f, T/I=%f", elapsedTime*1000000/total_events, elapsedTime*1000/iter);
    if (TP_one>7) TP_one=7;
    TP+=TP_one;
    ROS_INFO("TP=%d, LBI=%d, GT=%d", TP, LBI, GT); 
    float precision=float(TP)/float(GT)*100;
    float recall=float(LBI)/float(GT)*100;
    float F1=2*precision*recall/(precision+recall);
    ROS_INFO("precision=%f, recall=%f, F1=%f", precision, recall, F1);
    ROS_INFO("TP1=%d, LBI1=%d, GT1=%d", TP_one, LBI_one, GT_one);  
