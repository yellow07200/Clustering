...
void meanshiftCluster_Gaussian(cv::Mat dataPts, std::vector<double> *clusterCenterX, std::vector<double> *clusterCenterY, std::vector<double> *clusterCenterZ, std::vector<int> *point2Clusters, double bandwidth, cv::Mat TclusterVotes)
{
	...
	while (numInitPts>0)
	{
		...
		while(true)
		{
			...
			//std::cout << "G-114-2=" << (cv::sum(diff.row(0)+diff.row(1)+diff.row(2)))<<std::endl;
			//x1+y1+z1-x2-y2-z2;
			//---double dev_diff=(double)(cv::sum(diff.row(0)+diff.row(1)+diff.row(2))[0]);//sum(x-x')
			//cv::Mat dev_diff=diff;
			...
			cv::Mat weight;
			cv::Mat tempmat=diffdiff/lambda;
			cv::exp(tempmat, weight);
			...
			temp1 = myMeanX+0.55*(myMeanX-myOldMeanX);//dev_myMeanX; //0.5*(myMeanX-myOldMeanX);
			temp2 = myMeanY+0.55*(myMeanY-myOldMeanY);//dev_myMeanY; //0.5*(myMeanY-myOldMeanY);
			temp3 = myMeanZ+0.55*(myMeanZ-myOldMeanZ);//dev_myMeanZ; //0.5*(myMeanZ-myOldMeanZ);
...
*/
		...
		//ROS_ERROR_STREAM("numPts="<<numPts);
