...
#define bandwidth 0.15//0.15//0.17//0.15
...
Meanshift::Meanshift(ros::NodeHandle & nh) : nh_(nh) //, ros::NodeHandle nh_private//
{
	...
	//pcl_sub_=nh.subscribe<sensor_msgs::PointCloud2> ("/point_cloud_Clustering", 1, PointCloud2Callback);
...
void Meanshift::eventsCallback_simple(const dvs_msgs::EventArray::ConstPtr& msg)
{
	...
	if (events_size>60)
	{
		...
		while(beginEvent < events_size )
		{
			...
			cv::Mat data=cv::Mat(4, min(packet, events_size-beginEvent), CV_64F, cv::Scalar::all(0.));
			...
			for (int i = beginEvent; i < min(beginEvent+packet, events_size); i++)//
			{
				...
				data.at<double>(cv::Point(counterOut, 0))= event_timestamp/final_timestamp;
				data.at<double>(cv::Point(counterOut, 1))= (double)x/numCols;
				//data.at<double>(cv::Point(counter, 2))= event_timestamp/final_timestamp;//normalized
				data.at<double>(cv::Point(counterOut, 2))= (double)y/numRows;//normalized
				data.at<double>(cv::Point(counterOut, 3))= 0;
				double tau = 10000;
				//data.at<double>(cv::Point(counterOut, 2))= exp(-(final_timestamp-event_timestamp)/tau);//normalized
				activeEvents.at(counterIn)=true;
		
				data_cluster.at<int>(cv::Point(counterOut, 0))= 0;
				data_cluster.at<int>(cv::Point(counterOut, 1))= x;
				data_cluster.at<int>(cv::Point(counterOut, 2))= y;
				data_cluster.at<int>(cv::Point(counterOut, 3))= 0; //depth-
			...
			for (int i=0; i<clustCentX.size();i++)//auto
			{
				...
				//{
					...
					zz=std::floor(clustCentZ[i]);
					centroid.push_back({xx,yy,zz});
					...
					for (int j = 0; j < min(beginEvent+packet, events_size); j++)
					{
						...
						dis_depth=pow((z-zz),2); // to be filled 
	 ...
	 /* if (centroid_final.size()>0)
		...
		{ 
			...
			if (centroid_final.size()>cluster_center_final.size())
			{
				for (int tf=0; tf<centroid_final.size(); tf++)
				{
					...
					int sign_repeat=0;
					for (int tc=0; tc<cluster_center_final.size(); tc++)
					{
						int ccx=cluster_center_final[tc][0];
						int ccy=cluster_center_final[tc][1];
					...
					if (sign_repeat==cluster_center_final.size()) 
					{
		...
		ROS_INFO("Cluster_Center.size=%d, Cluster_Center_final.size=%d",cluster_center.size(),cluster_center_final.size());
		for (int w=0;w<cluster_center_final.size();w++)//centroid_final //cluster_center
		{
		...
		if (cluster_center_final.size()>0) 
			vec_far=FindFar(points_ID0, cluster_center_final[0][0], cluster_center_final[0][1], angle_ID0, imageID);//cv_image1.image);//
